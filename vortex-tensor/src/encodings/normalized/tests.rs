...
/// Snapshots a tensor-like array as `(dtype, per-row validity, per-element values)` so two columns
/// can be compared without depending on their physical encoding.
///
/// Elements belonging to null rows come back as `None`. A null row's physical storage values are
/// unspecified, so comparing them would pin an implementation detail rather than the column.
...
/// The push-down rules rebuild the array from sliced/filtered children, so they have to carry the
/// validity along with them.
#[test]
...
/// `scalar_at` collapses the row's norm to a one-element constant, which routes it through the
/// constant-norms path. A norm within `unit_norm_tolerance` of 1.0 must not be treated as the
/// identity there, or a per-row read would disagree with a bulk decode in the last bits.
#[test]
fn scalar_at_matches_bulk_decode_for_near_unit_norms() -> VortexResult<()> {
    let near_unit = 1.0f64 + 2.0 * f64::EPSILON;
    let normalized = vector_array(2, &[1.0f64, 0.0])?;
    let norms = PrimitiveArray::from_iter([near_unit]).into_array();

    let mut ctx = SESSION.create_execution_ctx();
    let normalized_array =
        Normalized::try_new(normalized, norms, Validity::NonNullable, &mut ctx)?.into_array();
    let bulk: ArrayRef = normalized_array.clone().execute(&mut ctx)?;

    let row = normalized_array.execute_scalar(0, &mut ctx)?;
    assert_eq!(row, bulk.execute_scalar(0, &mut ctx)?);

    // Both paths must have actually applied the norm, not just agreed on skipping it.
    let ext: ExtensionArray = bulk.execute(&mut ctx)?;
    let storage: FixedSizeListArray = ext.storage_array().clone().execute(&mut ctx)?;
    let elements: PrimitiveArray = storage.elements().clone().execute(&mut ctx)?;
    assert_eq!(elements.as_slice::<f64>()[0], near_unit);
...
/// The array carries no metadata: the parent dtype supplies the tensor shape, element ptype, and
/// nullability, both children's dtypes follow from it, and the validity child shows up in the child
/// count.
#[test]
...
fn serde_round_trip_preserves_the_stored_validity() -> VortexResult<()> {
    ...
    let mut ctx = SESSION.create_execution_ctx();
    let validity = Validity::from_iter([true, false]);
    let original = Normalized::try_new(normalized, norms, validity, &mut ctx)?.into_array();
    let children: Vec<ArrayRef> = original.children();
    let metadata = SESSION
        .array_serialize(&original)?
        .expect("Normalized must serialize");
...
/// The scheme reports `AlwaysUse`, so a canonical array it claims is never offered to another
/// scheme. Claiming a non-float tensor would abort the whole column's compression on the float-only
/// gate in `compress` rather than falling through.
#[rstest]
