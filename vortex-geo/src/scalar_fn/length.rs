...
fn validate_length_operands(dtypes: &[DType]) -> VortexResult<()> {
    vortex_ensure!(
        dtypes.len() == 1,
        "geo: length requires exactly one line string operand, got {}",
        dtypes.len()
    );
    vortex_ensure!(
        dtypes[0]
            .as_extension_opt()
            .is_some_and(|extension| extension.is::<crate::extension::LineString>()),
        "geo: length operand {} is not a native line string",
        dtypes[0]
    );
    Ok(())
}
...
impl GeoLength {
    ...
    pub fn try_new_array(array: ArrayRef) -> VortexResult<ScalarFnArray> {
        ScalarFnArray::try_new(
            TypedScalarFnInstance::new(GeoLength, EmptyOptions).erased(),
            vec![array],
        )
    }
}

impl ScalarFnVTable for GeoLength {
    type Options = EmptyOptions;

    fn id(&self) -> ScalarFnId {
        static ID: CachedId = CachedId::new("vortex.geo.length");
        *ID
    }
    ...
    fn execute(
        ...
        execute_unary_geo_types(
            ...
            |geometry| match geometry {
                ...
                _ => unreachable!("length input is validated as a line string"),
            },
            ctx,
        )
    }
...
mod tests {
    ...
    fn measures_each_linestring() -> VortexResult<()> {
        ...
        let lengths = GeoLength::try_new_array(lines)?
            .into_array()
            .execute::<Canonical>(&mut ctx)?
            .into_primitive();

        assert_eq!(lengths.as_slice::<f64>(), &[5.0, 9.0, 0.0]);
        Ok(())
    }
    ...
    fn constant_is_computed_once_and_remains_constant() -> VortexResult<()> {
        let session = vortex_array::array_session();
        let mut ctx = session.create_execution_ctx();
        let lines = line_constant(vec![(0.0, 0.0), (3.0, 4.0)], 3, &mut ctx)?;

        let result = GeoLength::try_new_array(lines)?
            .into_array()
        ...
        assert_eq!(f64::try_from(lengths.scalar())?, 5.0);
        Ok(())
    }
    ...
    fn rejects_non_linestring_dtype() -> VortexResult<()> {
        ...
        assert!(GeoLength.return_dtype(&EmptyOptions, &[primitive]).is_err());
        Ok(())
    }
}
