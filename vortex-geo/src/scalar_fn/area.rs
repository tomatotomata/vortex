...
use crate::scalar_fn::execute::execute_unary_null_propagating;

/// Validate the native geometry operand accepted by `ST_Area`.
fn validate_area_operand(dtypes: &[DType]) -> VortexResult<()> {
    vortex_ensure!(
        dtypes.len() == 1,
        "geo: area requires exactly one geometry operand, got {}",
        dtypes.len()
    );
    vortex_ensure!(
        is_native_geometry(&dtypes[0]),
        "geo: area operand {} is not a native geometry",
        dtypes[0]
    );
    Ok(())
}
...
impl GeoArea {
    ...
    pub fn try_new_array(array: ArrayRef) -> VortexResult<ScalarFnArray> {
        ScalarFnArray::try_new(
            TypedScalarFnInstance::new(GeoArea, EmptyOptions).erased(),
            vec![array],
        )
    }
}

impl ScalarFnVTable for GeoArea {
    type Options = EmptyOptions;

    fn id(&self) -> ScalarFnId {
        static ID: CachedId = CachedId::new("vortex.geo.area");
        *ID
    }
    ...
    fn execute(
        ...
        execute_unary_null_propagating(&array, Area::unsigned_area, ctx)
    }
...
mod tests {
    ...
    fn measures_non_polygon_native_geometries() -> VortexResult<()> {
        ...
        for (geometry, expected) in cases {
            let areas = GeoArea::try_new_array(geometry)?
                .into_array()
    ...
    fn measures_polygons_with_holes_and_empty_polygons() -> VortexResult<()> {
        ...
        let areas = GeoArea::try_new_array(polygons)?
            .into_array()
            .execute::<Canonical>(&mut ctx)?
            .into_primitive();
    ...
    fn measures_multipolygons() -> VortexResult<()> {
        ...
        let areas = GeoArea::try_new_array(multipolygons)?
            .into_array()
            .execute::<Canonical>(&mut ctx)?
            .into_primitive();
    ...
    fn propagates_nulls() -> VortexResult<()> {
        ...
        let areas = GeoArea::try_new_array(multipolygons)?.into_array();
        let expected =
    ...
    fn rejects_non_geometry_dtype() -> VortexResult<()> {
        ...
        assert!(GeoArea.return_dtype(&EmptyOptions, &[primitive]).is_err());
        Ok(())
    }
}
