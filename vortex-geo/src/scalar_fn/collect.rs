...
fn collect_dtype(dtypes: &[DType]) -> VortexResult<DType> {
    vortex_ensure!(
        dtypes.len() == 1,
        "geo: collect requires exactly one list operand, got {}",
        dtypes.len()
    );
    let DType::List(element_dtype, nullability) = &dtypes[0] else {
        vortex_bail!("geo: collect operand {} is not a list", dtypes[0]);
    };
    let Some(element) = element_dtype.as_extension_opt() else {
        vortex_bail!(
            "geo: collect list element {} is not a native Point, LineString, or Polygon",
            element_dtype
        );
    };
    ...
    let (offsets, sizes) = if compact_elements {
        ...
        for (&old_offset, &old_size) in old_offsets.iter().zip(old_sizes.iter()) {
            let start = usize::try_from(old_offset)
                .map_err(|_| vortex_err!("geo: collect element offset exceeds usize"))?;
            let size = usize::try_from(old_size)
                .map_err(|_| vortex_err!("geo: collect element count exceeds usize"))?;
            let end = start
                .checked_add(size)
                .ok_or_else(|| vortex_err!("geo: collect element range overflows usize"))?;
            vortex_ensure!(
                end <= element_valid.len(),
                "geo: collect element range {start}..{end} exceeds element length {}",
                element_valid.len()
            );
            let size = u64::try_from(valid_count(&element_valid, start, end))
                .map_err(|_| vortex_err!("geo: collect valid element count exceeds u64"))?;
            offsets.push(next_offset);
            sizes.push(size);
            next_offset = next_offset
                .checked_add(size)
                .ok_or_else(|| vortex_err!("geo: collect output offset exceeds u64"))?;
        }
...
impl GeoCollect {
    ...
    pub fn try_new_array(array: ArrayRef) -> VortexResult<ScalarFnArray> {
        ScalarFnArray::try_new(
            TypedScalarFnInstance::new(GeoCollect, EmptyOptions).erased(),
            vec![array],
        )
    }
}

impl ScalarFnVTable for GeoCollect {
    type Options = EmptyOptions;

    fn id(&self) -> ScalarFnId {
        static ID: CachedId = CachedId::new("vortex.geo.collect");
        *ID
    }
...
mod tests {
    ...
    fn all_valid_collect_reuses_geometry_storage() -> VortexResult<()> {
        ...
        let result = GeoCollect::try_new_array(input)?
            .into_array()
            .execute::<ExtensionArray>(&mut ctx)?;
    ...
    fn rejects_unsupported_inputs() -> VortexResult<()> {
        ...
        assert!(GeoCollect::try_new_array(point).is_err());

        let multipoints = multipoint_column(vec![vec![(0.0, 0.0)]])?;
        assert!(GeoCollect::try_new_array(list(multipoints, &[0, 1])?).is_err());
