...
fn validate_make_line_operands(dtypes: &[DType]) -> VortexResult<()> {
    vortex_ensure!(
        dtypes.len() == 2,
        "geo: make_line requires exactly two point operands, got {}",
        dtypes.len()
    );
    for dtype in dtypes {
        vortex_ensure!(
            ...
            "geo: make_line operand {dtype} is not a native point"
        );
    }
    Ok(())
}
...
fn make_line_metadata(left: &GeoMetadata, right: &GeoMetadata) -> VortexResult<GeoMetadata> {
    match (&left.crs, &right.crs) {
        (Some(left_crs), Some(right_crs)) => {
            vortex_ensure!(
                left_crs == right_crs,
                "geo: make_line operands have different coordinate reference systems: \
...
fn build_make_lines(
    ...
    let [start, end] = operands.map(|operand| match operand {
        Operand::Constant(point) => ConstantArray::new(point, len).into_array(),
        Operand::Column(points) => points,
    });
    let starts = flatten_coordinates(&start, ctx)?;
    let ends = flatten_coordinates(&end, ctx)?;
...
impl GeoMakeLine {
    ...
    pub fn try_new_array(a: ArrayRef, b: ArrayRef) -> VortexResult<ScalarFnArray> {
        ScalarFnArray::try_new(
            TypedScalarFnInstance::new(GeoMakeLine, EmptyOptions).erased(),
            vec![a, b],
        )
    }
}

impl ScalarFnVTable for GeoMakeLine {
    type Options = EmptyOptions;

    fn id(&self) -> ScalarFnId {
        static ID: CachedId = CachedId::new("vortex.geo.make_line");
        *ID
    }
...
mod tests {
    ...
    fn connects_paired_points_in_operand_order() -> VortexResult<()> {
        ...
        let lines = GeoMakeLine::try_new_array(starts, ends)?.into_array();
    ...
    fn two_constants_are_built_once_and_remain_constant() -> VortexResult<()> {
        ...
        let result = GeoMakeLine::try_new_array(starts, ends)?
            .into_array()
    ...
    /// A null endpoint produces a null line, which in turn produces a null length when the two
    /// scalar functions are composed.
    #[test]
    fn propagates_endpoint_nulls_through_length() -> VortexResult<()> {
        let session = vortex_array::array_session();
        let mut ctx = session.create_execution_ctx();
        let starts = nullable_point_column(vec![Some((0.0, 0.0)), None, Some((0.0, 0.0))])?;
        let ends = nullable_point_column(vec![Some((3.0, 4.0)), Some((1.0, 1.0)), None])?;
    ...
    fn promotes_mixed_point_dimensions(
        ...
        let lines = GeoMakeLine::try_new_array(start, end)?.into_array();
        ...
        for (name, expected) in [("z", expected_z), ("m", expected_m)] {
            if let Some(expected) = expected {
                assert_eq!(
                    ordinates(&vertices, name, &mut ctx)?
                        .iter()
                        .copied()
                        .collect::<Vec<_>>(),
                    expected
    ...
    fn propagates_compatible_crs(
        ...
        let dtype = GeoMakeLine.return_dtype(
            &EmptyOptions,
